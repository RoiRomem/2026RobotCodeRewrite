...
public class ShooterIORev implements ShooterIO {
    ...
    public ShooterIORev() {
        ...
        tryUntilOk(m_spark, 5,
                () -> m_spark.configure(m_config, ResetMode.kNoResetSafeParameters, PersistMode.kPersistParameters));

        SparkFlexConfig s_config = new SparkFlexConfig();
        s_config.follow(m_spark, ShooterConstants.Flywheel.kFollowerInverted);

        tryUntilOk(m_spark, 5,
                () -> s_spark.configure(s_config, ResetMode.kNoResetSafeParameters, PersistMode.kPersistParameters));
